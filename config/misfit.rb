module Misfit
  module Config
    ...
    @crud_rights = {
      ...
      :data_entry => {
        :all => [:client, :loan, :payment, :document, :client_group, :group, :insurance_company, :insurance_policy, :cgt, :grt, :staff_member, :guarantor],
      },
      :staff_member => {
        :all => [:center, :client, :loan, :payment, :document, :client_group, :group, :comment, :insurance_company, :staff_member, :location, :branch_diary, :guarantor, :stock_register, :asset_register]
      },
      :funder => {
        :all => []
      },
    ...
    @access_rights = {
      ...
      :data_entry => {
        :all => [:search, :comments, :documents, :"data_entry/client_groups", :"data_entry/payments", :"data_entry/clients",:"data_entry/loans", :"data_entry/index", 
                 :clients, :loans, :client_groups, :cgts, :grts, :staff_members, :guarantors, :bookmarks]
      },
      :read_only => {
      ...
      :staff_member => {
        :all => [:documents, :searches, :browse, :branches, :centers, :payments, :clients, :client_groups, :groups, :audit_trails, :comments, :insurance_policies, 
                 ...
                 :"data_entry/loans", :"data_entry/index", :insurance_companies, :info, :dashboard, :graph_data, :branch_diaries, :stock_registers, :asset_registers, :bookmarks, :loan_products, :holidays]
      },
      :funder => {
    ...
    module DateFormat
      def self.compile(mfi = Mfi.first)
        if format = mfi.date_format and not format.blank? and Mfi::DateFormats.include?(format)
          Date.class_eval do
            format = mfi.date_format
