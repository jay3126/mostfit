  ...
  task :client_wise_outstanding_report do
    ...
    loan_ids.each do |l|
      ...
      if (loan.status == :outstanding)
        ...
        f.puts("#{sl_no}, #{branch_id}, \"#{branch_name}\", #{center_id}, \"#{center_name}\", #{client_id}, \"#{client_name}\", \"#{client_gender}\", #{loan_id}, #{loan_amount}, \"#{loan_status}\", #{loan_cycle_number}, #{loan_interest_rate}, #{loan_disbursal_date}, #{loan_number_of_installments}, \"#{loan_installment_frequency}\", \"#{loan_product}\", #{principal_outstanding_as_on_date1}, #{principal_outstanding_as_on_date2}, #{principal_outstanding_as_on_date3}, #{interest_outstanding_as_on_date1}, #{interest_outstanding_as_on_date2}, #{interest_outstanding_as_on_date3}")
      end
    end
    f.close
  end
