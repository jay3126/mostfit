    ...
    def generate_collection_pdf(date)
      ...
      weeksheets.each do |weeksheet|
        ...
        weeksheet.groups.each do |group|
          ...
          table.data.push({"disbursed" => group ? group[1] : "No group"})
          loan_row_count=0
          collection_sheets.each do |ws|
            table.data.push({"name" => ws.borrower_name, "loan id" => ws.loan_id, "amount" => ws.loan_disbursed_amount.to_currency,
                "outstanding" => ws.loan_outstanding.to_currency, "status" => ws.loan_status,
                "disbursed" => ws.loan_disbursed_date, "installment" => ws.loan_installment_number,
                "principal" => ws.principal_due_today.to_currency, "interest" => ws.interest_due_today.to_currency,
                "days absent/total" => (days_absent[ws.borrower_id]||0).to_s / (days_present[ws.borrower_id]||0).to_s,
                "fee" => ws.fee_due_today.to_currency, "total due" =>  ws.total_due.to_currency, "signature" => "" })
            group_amount       += ws.loan_disbursed_amount
            group_outstanding  += ws.loan_outstanding
            group_installments += ws.loan_installment_number
            group_principal    += ws.principal_due_today
            group_interest     += ws.interest_due_today
            group_fee          += ws.fee_due_today
            group_due          += ws.total_due
            loan_row_count     += 1
            if loan_row_count==0
