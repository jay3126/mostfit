  ...
  def self.get_clients_at_location_by_sql(administered_or_registered_choice, given_location_id, on_date = Date.today, count = false)
    ...
    if client_ids.blank?
      ...
      if count
        ...
        client_ids = repository(:default).adapter.query("select ca.counterparty_id from (select #{administered_or_registered_choice}, counterparty_id, counterparty_type from client_administrations where counterparty_type = 1 AND counterparty_id IN (#{client_ids.join(',')})) ca where #{administered_or_registered_choice} = (select #{administered_or_registered_choice} from (select * from client_administrations where counterparty_type = 1 AND counterparty_id IN (#{client_ids.join(',')})) ca1 where ca.counterparty_id = ca1.counterparty_id and ca.counterparty_type = 1 and ca.#{administered_or_registered_choice} IN (#{locations[administered_or_registered_choice].join(',')}) order by ca1.effective_on desc limit 1 );")
