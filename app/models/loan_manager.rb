class LoanManager
  ...
  def get_loan_status_on_date(for_loan_id, on_date)
    loan = Lending.get(for_loan_id)
    raise Errors::DataMissingError, "Unable to locate loan with id: #{for_loan_id}" unless loan
    loan.loan_status_on_date(on_date)
  end
  ...
  def get_loan_due_status_on_date(for_loan_id, on_date)
    loan = Lending.get(for_loan_id)
    raise Errors::DataMissingError, "Unable to locate loan with id: #{for_loan_id}" unless loan
    loan.due_status_on_date(on_date)
  end
  ...
  def amounts_received_till_date(on_loan_id, on_or_before_date)
    loan = Lending.get(on_loan_id)
    raise Errors::DataMissingError, "Unable to locate loan with id: #{on_loan_id}" unless loan
    loan.amounts_received_till_date(on_or_before_date)
  end

  def principal_received_till_date(on_loan_id, on_or_before_date)
    loan = Lending.get(on_loan_id)
    raise Errors::DataMissingError, "Unable to locate loan with id: #{on_loan_id}" unless loan
    loan.principal_received_till_date(on_or_before_date)
  end

  def interest_received_till_date(on_loan_id, on_or_before_date)
    loan = Lending.get(on_loan_id)
    raise Errors::DataMissingError, "Unable to locate loan with id: #{on_loan_id}" unless loan
    loan.interest_received_till_date(on_or_before_date)
  end

  def advance_received_till_date(on_loan_id, on_or_before_date)
    loan = Lending.get(on_loan_id)
    raise Errors::DataMissingError, "Unable to locate loan with id: #{on_loan_id}" unless loan
    loan.advance_received_till_date(on_or_before_date)
  end
