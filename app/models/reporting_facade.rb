class ReportingFacade < StandardFacade
  ...
  def sum_all_loans_balances_at_accounted_locations_on_date(on_date, *at_location_ids_ary)
    ...
    at_location_ids_ary.each do |location_id|
      ...
      till_on_loan_receipts = repository(:default).adapter.query(" SELECT SUM(principal_received) as principal, SUM(interest_received) as interest, SUM(advance_received) as advance, SUM(advance_adjusted) as advance_adjustment, SUM(loan_recovery) as recovery FROM loan_receipts where lending_id IN (#{loans_ids.join(',')}) AND effective_on <= '#{on_date.strftime('%Y-%m-%d')}'").first
      loan_receipts         = repository(:default).adapter.query(" SELECT SUM(principal_received) as principal, SUM(interest_received) as interest, SUM(advance_received) as advance, SUM(advance_adjusted) as advance_adjustment, SUM(loan_recovery) as recovery FROM loan_receipts where lending_id IN (#{loans_ids.join(',')}) AND effective_on = '#{on_date.strftime('%Y-%m-%d')}'").first
      ...
      loan_amounts[location_id]['interest_amt']            = loan_receipts.interest.blank? ? MoneyManager.default_zero_money : MoneyManager.get_money_instance_least_terms(loan_receipts.interest.to_i)
      loan_amounts[location_id]['advance_amt']             = loan_receipts.advance.blank? ? MoneyManager.default_zero_money : MoneyManager.get_money_instance_least_terms(loan_receipts.advance.to_i)
      loan_amounts[location_id]['advance_adjustment_amt']  = loan_receipts.advance_adjustment.blank? ? MoneyManager.default_zero_money : MoneyManager.get_money_instance_least_terms(loan_receipts.advance_adjustment.to_i)
      loan_amounts[location_id]['recovery_amt']            = loan_receipts.recovery.blank? ? MoneyManager.default_zero_money : MoneyManager.get_money_instance_least_terms(loan_receipts.recovery.to_i)
  ...
  def sum_all_loans_balances_at_accounted_locations_for_date_range(on_date, till_date, *at_location_ids_ary)
    ...
    at_location_ids_ary.each do |location_id|
      ...
      loan_preclose         = repository(:default).adapter.query(" SELECT SUM(principal_received) as principal, SUM(interest_received) as interest FROM loan_receipts where lending_id IN (#{preclose_loan_ids.join(',')}) AND effective_on >= #{on_date} AND effective_on <= #{till_date}").first
