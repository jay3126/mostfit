class MeetingInfo
  ...
  def to_s
    "Meeting at (#{location_id}) on #{on_date} at #{meeting_begins_at}"
  end
...
class MeetingCalendar
  ...
  property :id,                         Serial
  ## that is useless attribute according to new biz_location improvements
  ...
  def self.setup_calendar(on_date, for_locations_and_schedules)
    ...
    meeting_map = {}
    for_locations_and_schedules.each do |id, meeting_schedule|

      if meeting_schedule.is_proposed_scheduled_on_date?(on_date)
        meeting_map[id] = meeting_schedule
        locations_and_meetings << meeting_map
      end
    end
  ...
  def self.setup(locations_and_meetings, on_date)
    locations_and_meetings.each { |meeting_map|
  ...
  def self.predicates_for_location(location)
    raise ArgumentError, "a location id was not specified" unless location.location_id
