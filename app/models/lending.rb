class Lending
  ...
  def self.total_loans_on_date(status = nil, on_date = Date.today)
    status_key = status.blank? ? '' : LoanLifeCycle::LOAN_STATUSES.index(status.to_sym)
    if status_key.blank?
