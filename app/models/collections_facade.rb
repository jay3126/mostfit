...
class CollectionsFacade < StandardFacade
  ...
  def get_collection_sheet_for_location(at_biz_location, on_date, only_installment_date = false)
    collection_sheet_line = []
    zero_amt              = MoneyManager.default_zero_money
    on_date = on_date.class == Date ? on_date : Date.parse(on_date)
    biz_location    = BizLocation.get(at_biz_location)
    ...
    collection_sheet_line                    = []
    loans                                    = only_installment_date ? Lending.all(:id => loan_ids, "loan_base_schedule.base_schedule_line_items.on_date" => on_date) : Lending.all(:id => loan_ids, "loan_base_schedule.base_schedule_line_items.on_date".to_sym.lte => on_date)
