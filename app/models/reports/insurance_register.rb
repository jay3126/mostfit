class InsuranceRegister < Report
  ...
  def generate
    r = repository.adapter.query(%Q{
                                    ...
                                    WHERE l.client_id = c.id AND g.client_id = c.id AND c.occupation_id = o.id AND l.c_branch_id = b.id AND l.occupation_id = o.id AND l.disbursal_date >= '#{from_date.strftime('%Y-%m-%d')}' AND l.disbursal_date <= '#{to_date.strftime('%Y-%m-%d')}' AND c.active = true AND b.id = #{@branch_id}
