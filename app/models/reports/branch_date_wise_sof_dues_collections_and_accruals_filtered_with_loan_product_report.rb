class BranchDateWiseSofDuesCollectionsAndAccrualsFilteredWithLoanProductReport < Report
  attr_accessor :from_date, :to_date, :lending_product_id

  validates_with_method :lending_product_id, :loan_product_should_be_selected

  def initialize(params, dates, user)
    @from_date = (dates and dates[:from_date]) ? dates[:from_date] : Date.today - 7
    @to_date   = (dates and dates[:to_date]) ? dates[:to_date] : Date.today
    @name = "Branch Date Wise SOF Dues Collections and Accruals Report Filtered with Loan Product from #{@from_date} to #{@to_date}"
    @user = user
    if (params and params[:lending_product_id] and (not (params[:lending_product_id].empty?)))
  ...
  def name
    "Branch Date Wise SOF Dues Collections and Accruals Report Filtered with Loan Product from #{@from_date} to #{@to_date}"
  end

  def self.name
    "Branch Date Wise SOF Dues Collections and Accruals Report Filtered with Loan Product"
  end
  ...
  def generate
    ...
    loan_ids = Lending.all(:lending_product_id => @lending_product_id).aggregate(:id)
    ...
    loan_ids.each do |loan_id|
      ...
      loan_product = LendingProduct.get(loan.lending_product_id)
      loan_product_name = loan_product ? loan_product.name : "Not Specified"
      product_id = loan_product ? loan_product.id : "Not Specified"
      ...
      (@from_date..@to_date).each do |on_date|
        ...
        data[product_id][on_date] = {
          :loan_product_name => loan_product_name, :product_id => product_id, :on_date => on_date,
  ...
  def loan_product_should_be_selected
    return [false, "Please select Loan Product"] if self.respond_to?(:lending_product_id) and not self.lending_product_id
    return true
  end
