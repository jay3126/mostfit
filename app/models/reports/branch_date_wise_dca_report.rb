  ...
  def generate
    ...
    data[:branches].each do |branch_id|
      ...
      (@from_date..@to_date).each do |on_date|
        ...
        loan_receipt_on_date = loan_receipts.select{|s| s.effective_on == on_date}
        preclose_loan_receipt_on_date = preclose_loan_receipts.select{|s| s.effective_on == on_date}
        ...
        loan_receipts_till_date = loan_receipts.select{|r| r.effective_on <= on_date}
        loan_receipts_amt_till_date = LoanReceipt.add_up(loan_receipts_till_date)
