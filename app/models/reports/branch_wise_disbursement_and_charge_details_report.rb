class BranchWiseDisbursementAndChargeDetailsReport < Report
  ...
  attr_accessor :date, :biz_location_branch_id, :lending_product_id
  ...
  def initialize(params, dates, user)
    ...
    @lending_product_id = (params and params[:lending_product_id] and (not (params[:lending_product_id].empty?))) ? params[:lending_product_id] : LendingProduct.all.map{|lp| lp.id}
  ...
  def generate
    ...
    at_branch_ids_ary.each do |branch_id|
      ...
      loan_disbursals = reporting_facade.loans_disbursed_by_branches_and_lending_products_on_date(@date, lending_product, *branch_id)
