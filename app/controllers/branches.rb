class Branches < Application
  ...
  def centers
    if params[:id] 
      ...
      return("<option value=''>Select center</option>") unless branch
      centers = params[:paying] ? Center.paying_today(session.user, Date.parse(params[:date]), params[:id]) : branch.centers(:order => [:name]) 
      return("<option value=''>Select center</option>"+centers.map{|cen| "<option value=#{cen.id}>#{cen.name}</option>"}.join)
    end
  end
