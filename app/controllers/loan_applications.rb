class LoanApplications < Application
          ...
          if error_msg.blank?
            begin
              loan_application = LoanApplication.new({:at_branch_id => branch_id,
                  ...
                  :client_name => name,
                  :client_age_as_on => client_age_as_on,
  ...
  def list
    ...
    @errors << "Created on date must not be future date" if Date.parse(created_on) > get_effective_date
    ...
    if @errors.blank?
      ...
      client_ids.each do |client_id|
        begin
          unless clients[client_id][:selected].blank?
            selected += 1
  ...
  def bulk_create_loan_applicant
    ...
    @message[:error] << "Created on date must not be future date" if Date.parse(created_on) > get_effective_date
  ...
  def update
    ...
    begin
      ...
      if crucial_info_changed && @loan_application.status != Constants::Status::LOAN_FILE_GENERATED_STATUS && @loan_application.status != Constants::Status::NEW_STATUS && !Constants::Status::CPV_STATUSES.include?(@loan_application.status)
  ...
  def duplicate_record
    ...
    if !reference1.blank? && !reference2.blank?
      @clients           = Client.all(:conditions => ["reference like ? or reference2 like ?", reference1, reference2])
      @clients           = Client.all(:conditions => ["reference like ? or reference2 like ?", no_reference1, no_reference2]) if @clients.blank?
      ...
      loan_app_condition = {:conditions => ["client_reference1 like ? or client_reference2 like ?", reference1, reference2]}
    else
      if !reference1.blank?
        @clients           = Client.all(:conditions => ["reference like ?", reference1])
        @clients           = Client.all(:conditions => ["reference like ?", no_reference1]) if @clients.blank?
        @clients           = Client.all(:conditions => ["reference LIKE ? or reference LIKE ?", "%#{no_reference1}%", "%#{no_reference1}%"]) if @clients.blank?
        loan_app_condition = {:conditions => ["client_reference1 like ?", reference1]}
      end

      if !reference2.blank?
        @clients           = Client.all(:conditions => ["reference2 like ?", reference2])
        @clients           = Client.all(:conditions => ["reference2 like ?", no_reference2]) if @clients.blank?
        @clients           = Client.all(:conditions => ["reference2 LIKE ? or reference2 LIKE ?", "%#{reference2}%", "%#{no_reference2}%"]) if @clients.blank?
        loan_app_condition = {:conditions => ["client_reference2 like ?", reference2]}
      end
    end
