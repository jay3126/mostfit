    ...
    if @errors.blank?
      params[:status].keys.each do |lap|
        begin
          ...
          raise Errors::DataError, "Authorized on date(#{on_date}) must not before credit bureau rated on date(#{loan_application.credit_bureau_rated_at.display})" if Date.parse(on_date) < Date.parse(loan_application.credit_bureau_rated_at.display)
